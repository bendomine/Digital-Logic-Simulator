...
function evaluateFromData(index, input) {
	...
	while (blockQueue.length > 0) {
		currentEvaluations++;
		if (currentEvaluations > MAX_EVALUATIONS) {
			alert('Evaluation limit reached. Please check for loops in your circuit.');
			break;
		}
		...
		for (let i = 0; i < blockQueue[0].outPins.length; ++i) {
			if (blockQueue[0].outPins[i] != null) {
				for (let k = 0; k < blockQueue[0].connections[i].length; ++k) {
					components[blockQueue[0].connections[i][k][0]].inPins[
						[blockQueue[0].connections[i][k][1]]
					] = blockQueue[0].outPins[i];
...
function evaluate(event, extraBlocks) {
	...
	while (blockQueue.length > 0) {
		...
		for (let i = 0; i < blockQueue[0].inPins.length; ++i) {
			if (
				blockQueue[0].inPins[i] != null &&
				!blockQueue[0].inPins[i].active &&
				blockQueue[0].inPins[i].ref.classList.contains('isActive')
			)
				blockQueue[0].inPins[i].ref.classList.remove('isActive');
		}
		for (let i = 0; i < blockQueue[0].outPins.length; ++i) {
			if (blockQueue[0].outPins[i] != null) {
				if (blockQueue[0].outPins[i].active)
					blockQueue[0].outPins[i].ref.classList.add('isActive');
