	...
	for (let i = 0; i < blocks.length; ++i) {
				...
				if (blocks[i].outPins[j] != null)
					for (let k = 0; k < blocks[i].outPins[j].connected.length; ++k) {
						if (blocks[i].outPins[j].connected[k].ref != null) {
							...
							x2 =
								blocks[i].outPins[j].connected[k].ref.getBoundingClientRect()
									.right - 5;
							...
							gradient.addColorStop(
								1,
								blocks[i].outPins[j].connected[k].ref.parentElement.style
									.backgroundColor
							);
							if (
								blocks[i].outPins[j].active ||
								blocks[i].outPins[j].connected[k].active
							)
		...
		//		lines to the mouse cursor at once, but if that somehow does happen, I want to know about it.
			...
			for (let j = 0; j < blocks[i].pin.connected.length; ++j) {
				...
				if (blocks[i].pin.connected[j].active)
					// ctx.strokeStyle =
					// 	blocks[i].pin.connected[j].ref.parentElement.style.backgroundColor;
...
function newWire(origin) {
	...
	document.onmousemove = updateLines;
	...
	// Future me here: It's been multiple years since I wrote this, and I have no idea why this is so complicated. I'm not gonna touch it though.
	document.onmouseup = () => {
		document.onmousedown = (e2) => {
			endWire(origin, e2);
		};
